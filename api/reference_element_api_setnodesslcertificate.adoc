---
permalink: api/reference_element_api_setnodesslcertificate.html 
sidebar: sidebar 
keywords: ssl,certificate,set,setting,private,key,setnodesslcertificate 
summary: Sie können die Methode SetNodeSSLCertificate verwenden, um ein Benutzer-SSL-Zertifikat und einen privaten Schlüssel für den Management-Knoten festzulegen. 
---
= SetNodeSSLZertifikat
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
Sie können das verwenden `SetNodeSSLCertificate` Methode zum Festlegen eines Benutzer-SSL-Zertifikats und eines privaten Schlüssels für den Management-Node.



== Parameter


NOTE: Sie müssen diese Methode für den Management-Node anrufen. Beispiel:

[listing]
----
https://<management node IP>:442/json-rpc/10.0
----
Diese Methode verfügt über die folgenden Eingabeparameter:

|===
| Name | Beschreibung | Typ | Standardwert | Erforderlich 


 a| 
Zertifikat
 a| 
Die PEM-kodierte Textversion des Zertifikats.*Hinweis:* beim Festlegen eines Node- oder Cluster-Zertifikats muss das Zertifikat die Erweiterung DECDKeyUsage für serverAuth enthalten. Mit dieser Erweiterung kann das Zertifikat ohne Fehler auf gängigen Betriebssystemen und Browsern verwendet werden. Wenn die Erweiterung nicht vorhanden ist, weist die API das Zertifikat als ungültig zurück.
 a| 
Zeichenfolge
 a| 
Keine
 a| 
Ja.



 a| 
PrivateKey
 a| 
Die PEM-codierte Textversion des privaten Schlüssels.
 a| 
Zeichenfolge
 a| 
Keine
 a| 
Ja.

|===


== Rückgabewerte

Diese Methode hat keine Rückgabewerte.



== Anforderungsbeispiel

Anforderungen für diese Methode sind ähnlich wie das folgende Beispiel:

[listing]
----
{
    "method" : "SetNodeSSLCertificate",
    "params" : {
        "privateKey": "-----BEGIN RSA PRIVATE KEY-----\\nInDXvTgXMdMoet4BKnftelrXFKHgGqXJoczq4JWzGSIHNgvkrH60\n-----END RSA PRIVATE KEY-----\n",
        "certificate": "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n"
    },
    "id" : 2
}
----


== Antwortbeispiel

Diese Methode gibt eine Antwort zurück, die dem folgenden Beispiel ähnelt:

[listing]
----
{
    "id" : 2,
    "result" : {}
}
----